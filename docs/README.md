...
+ [ ] 방문 날짜, 주문 메뉴 등을 갖는 레코드 클래스
+ [ ] 총 주문 금액을 리턴하는 기능
+ [ ] 이벤트 혜택을 받기위한 최소금액(만원)을 넘겼는지를 리턴하는 기능
...
+ VisitDate interface
    + [ ] 날짜에 대한 유효성 검증하는 기능
    + [ ] 해당 날짜가 주말인지를 리턴하는 기능
    + [ ] 해당 날짜가 달력에 별이 있는 날인지를 리턴하는 기능
    + [ ] 해당 날짜가 디데이 할인 기간에 해당하는지 리턴하는 기능

+ DecemberVisitDate
    + [ ] VisitDate의 구현체로 방문 날짜(int) 속성을 갖는다
    + [ ] 날짜에 대한 유효성 검증
        + [ ] 1<= date <= 31 인지 검증
    + [ ] 해당 날짜가 12월 중 주말인지 리턴하는 기능
    + [ ] 해당 날짜가 12월 달력에 별이 있는 날인지 리턴하는 기능
    + [ ] 해당 날짜가 크리스마스 디데이할인 기간인지 리턴하는 기능

3. OrderMenu

> 고객이 주문한 메뉴와 갯수를 관리하는 도메인

+ [ ] 속성으로 HashMap<String, int> 를 갖고 고객이 주문한 메뉴, 갯수를 가진다.
+ [ ] 유효성을 검증한다.
    + [ ] 메뉴판에 존재하는 메뉴인지?
    + [ ] 메뉴의 개수가 1 이상의 값인지?
    + [ ] 중복 메뉴가 존재하는지?
+ [ ] 카테고리별 메뉴의 수량을 리턴하는 기능
...
+ DdayEventPolicy
    + [ ] EventPolicy 구현체
    + [ ] 이벤트를 적용받을 수 있는지를 리턴하는 기능
        + [ ] 12/1 , 12/25안에 고객이 방문했는지 검증
        + [ ] 총 주문 금액이 만원 이상인지 검증
    + [ ] 이벤트 적용 혜택금액을 리턴받는 기능
        + [ ] 이벤트 적용이 가능하다면, 크리스마스날에 다가갈수록 1000원에서부터 100원씩 할인
...
+ WeekDayEventPolicy
    ...
    + [ ] 이벤트를 적용받을 수 있는지를 리턴하는 기능
        ...
        + [ ] 총 주문 금액이 만원 이상인지 검증
    + [ ] 이벤트 적용 혜택금액을 리턴받는 기능
        + [ ] 이벤트 적용이 가능하다면, 디저트 메뉴 * 2023 원 할인
